...
const Todo = ({ id, label, tags, deadline, done }: todo) => {
  ...
  if (deadline) {
    ...
    if (!isNaN(dateObj.getTime())) {
      dateStr = new Intl.DateTimeFormat("fr-FR", { dateStyle: "full" }).format(dateObj);
    }
  }

  return (
    <li className={style.todo} aria-label={`todo : ${label}`}>
      <input type="checkbox" name="done" aria-label={`check ${label}`} checked={done} className={style.check} onChange={() => mutation.mutate({ done: !done })} />
      <div className="info">
